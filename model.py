...
#import torchtoolbox.transform as transform
...
class UNet(nn.Module):
    def __init__(self, num_classes, input_channels=3):
        ...
        # Upsampling
        self.up_concat3_1 = Unet_block(num_filter[3] + num_filter[4], num_filter[3], num_filter[3])
        self.up_concat2_2 = Unet_block(num_filter[2] + num_filter[3], num_filter[2], num_filter[2])
        self.up_concat1_3 = Unet_block(num_filter[1] + num_filter[2], num_filter[1], num_filter[1])
        self.up_concat0_4 = Unet_block(num_filter[0] + num_filter[1], num_filter[0], num_filter[0])

        self.last = nn.Conv2d(num_filter[0], num_classes, kernel_size=1)
    ...
    def forward(self, x):                    # (Batch, 3, 256, 256)
        x0_0 = self.conv0_0(x)               # (Batch, 32, 256, 256)
        x1_0 = self.conv1_0(self.pool(x0_0)) # (Batch, 64, 128, 128)
        x2_0 = self.conv2_0(self.pool(x1_0)) # (Batch, 128, 64, 64)
        x3_0 = self.conv3_0(self.pool(x2_0)) # (Batch, 256, 32, 32)
        x4_0 = self.conv4_0(self.pool(x3_0)) # (Batch, 512, 16, 16)

        x3_1 = self.up_concat3_1(torch.cat([x3_0, self.up(x4_0)], dim=1))  # concat[(Batch, 256, 32, 32), (Batch, 512, 32, 32) --> Conv(Batch, 768, 32, 32) --> (Batch, 256, 32, 32)
        x2_2 = self.up_concat2_2(torch.cat([x2_0, self.up(x3_1)], dim=1))  # concat[(Batch, 128, 64, 64), (Batch, 256, 64, 64) --> Conv(Batch, 384, 64, 64) --> (Batch, 128, 64, 64)
        x1_3 = self.up_concat1_3(torch.cat([x1_0, self.up(x2_2)], dim=1))  # concat --> Conv(Batch, 192, 128, 128) --> (Batch, 64, 128, 128)
        x0_4 = self.up_concat0_4(torch.cat([x0_0, self.up(x1_3)], dim=1))  # concat --> Conv(Batch, 96, 256, 256)  --> (Batch, 32, 256, 256)

        output = self.last(x0_4)   # (Batch, last_channels, 256, 256)
        return output
